defmodule Erlangjobs.Telegram do
  ...
  def post_offer(job, url) do
    ...
    *#{job.title}*
    ...
    {:ok, %Nadia.Model.Message{}} =
      apply(telegram_client(), :send_message, [jobs_chat_id(), job_text, [parse_mode: "Markdown"]])
  end
  ...
  def notify_new_offer(job, url) do
    ...
    *#{job.title}*
    ...
    """
    {:ok, %Nadia.Model.Message{}} =
      apply(telegram_client(), :send_message, [admins_chat_id(), job_text, [parse_mode: "Markdown"]])
  end
